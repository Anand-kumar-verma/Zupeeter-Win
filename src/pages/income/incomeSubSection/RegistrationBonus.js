...
function RegistrationBonus() {
  ...
  const { isLoading, data } = useQuery(
    ["level_bonus"],
    () => apiConnectorGet(endpoint?.level_income),
    {
            ...
            <TableBody>
              {visibleRows?.map((i, index) => (
                <TableRow key={i?.id}>
                  ...
                  <TableCell sx={{ color: 'white' }} className="!border !border-r !text-xs !text-center !border-b !border-white">{i?.l01_transection_type === "Level Income" ? "Team Trading Income" : i?.l01_transection_type}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
