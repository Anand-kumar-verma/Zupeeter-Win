...
//     return [
//       <span>{index + 1}</span>,
//       <span><Edit className="!text-green-500"/></span>,
...
const Player = () => {
  ...
  const UpdatePlayerFn = async (id) => {
    try {
      const res = await axiosInstance.post(
        `${API_URLS?.update_user_name}`,
        { u_user_id: id, u_user_name: Name }
      );
      toast.success(res?.data?.msg);
      setPlayer(null);
      userListFunction();
  ...
  const tablerow = data?.map((i, index) => {
    return [
      ...
      //         name="user_name"
      //         id={"user_name"}
      //         value={fk.values.user_name || i?.full_name}
      //         onChange={fk.handleChange}
      //       />
      //       <Button onClick={() => UpdatePlayerFn(i?.id)} variant="contained" color="primary" className="!text-white">
      //         Update
      //       </Button>
      //     </>
      //   ) : (
